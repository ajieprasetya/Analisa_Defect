...
function saveDataToDatabase($vin, $areaPart, $defect, $pic, $model, $analisis) {
    global $conn;

    // Periksa apakah VIN sudah ada di histori
    $sqlCheckVin = "SELECT * FROM input_2 WHERE `vin` = '$vin' AND `area/part` = '$areaPart' AND `defect` = '$defect' AND `pic` = '$pic' AND `model` = '$model'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["file"]["tmp_name"], $targetFile)) {
            ...
            $sqlInsertData = "INSERT INTO input_2 (`vin`, `area/part`, `defect`, `pic`, `model`, `analisis`) VALUES ('$vin', '$areaPart', '$defect','$pic', '$model', '$analisis')";
            ...
            if ($vin && $areaPart && $defect && $pic && $model) {
                saveDataToDatabase($vin, $areaPart, $defect, $pic, $model, basename($_FILES["file"]["name"]));
...
<body style="background-color: #0E1D35;">
    ...
    <div class="form-container">
        ...
        if (isset($_FILES["file"]["name"])) {
            ...
            echo "<a style=color: #82cfeb ; href='halaman_baru.php?vin=$vin&areaPart=$areaPart&defect=$defect&pic=$pic&model=$model'>Lihat</a>";
        }
        ?>
    </div>
